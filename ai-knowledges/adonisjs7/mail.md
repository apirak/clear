...
await mail.send((message) => {
  message.subject('Your order has shipped').from('orders@example.com')
})
```
...
await mail.send((message) => {
  message.to(user.email).cc(user.manager.email).bcc('audit@example.com')
})
```
...
await mail.send((message) => {
  message.from('noreply@example.com').replyTo('support@example.com')
})
```
...
await mail.send((message) => {
   ...
   * From a local file
   */
  message.icalEventFromFile(app.resourcesPath('invites/standup.ics'), {
    method: 'REQUEST',
   ...
   * From a URL
   */
  message.icalEventFromUrl('https://example.com/calendar/event-123.ics', {
    method: 'REQUEST',
    filename: 'event.ics',
  })
})
...
```ts title="app/controllers/notifications_controller.ts"
await mail.send((message) => {
  message.preparedHeader('X-Custom-Data', 'value with special chars or very long content')
})
```
...
export function postmarkTransport(config: PostmarkConfig): MailManagerTransportFactory {
