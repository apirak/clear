...
export default class UsersController {
  ...
  // [!code highlight:6]
  @inject()
  async sendEmail(ctx: HttpContext, userService: UserService, emailService: EmailService) {
...
router.resource('posts', controllers.Posts).as('articles')
```
...
router.resource('posts', controllers.Posts).params({ posts: 'post' })
```
